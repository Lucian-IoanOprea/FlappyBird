...
( define-struct mypipe (x gap-y)  #:transparent)
( define-struct myvariables(momentum gravity scroll-speed  ) #:transparent)
( define-struct mystate( pasare pipes vars score)#:transparent)


(define (get-initial-state)
   ( define flappyinitial (mybird bird-x bird-initial-y 0))
   ( define  mypipeinitial (mypipe scene-width (+ added-number (random random-threshold))))
   ( define myvariablesinitial (myvariables initial-momentum initial-gravity 0))
   (define state-initial (mystate flappyinitial (list mypipeinitial) myvariablesinitial 0))
  state-initial
  )
;TODO 2
...
(define (next-state-bird bird gravity)
 (struct-copy mybird bird [y (+ (mybird-y bird) (mybird-v-y bird))] [v-y (+ (mybird-v-y bird) gravity)])
...
(define (change current-state pressed-key)
 (match-let ([(mystate pasare pipes variables score ) current-state])
    (cond [(key=? pressed-key " ")  (struct-copy mystate current-state [pasare (next-state-bird-onspace (get-bird current-state) initial-momentum) ])]
          [else current-state]))
  )
...
(define (move-pipes pipes scroll-speed)
  ( map ( lambda( pipe )
           (struct-copy mypipe pipe [x (- (get-pipe-x pipe) scroll-speed) ]
            )
           )
        pipes
  )
...
(define (best-x pipes)
    (cond
       [(empty? pipes) 0]
       [(cons? pipes) (foldr max 0 (getter pipes))]))


(define (add-more-pipes pipes)
   (cond [ (< (length pipes) no-pipes ) 
                              (cons  (mypipe ( + pipe-width (+ ( best-x pipes) pipe-gap )) (+ added-number (random random-threshold)))  pipes   )]
          [else pipes]
...
(define (check-pipe-collisions bird pipes)
    (  if (null? pipes)
         #f
        (if ( or ( check-collision-rectangles (make-posn ( mybird-x bird ) ( get-bird-y bird )  )
                                               (make-posn  (+ ( mybird-x bird ) bird-width ) (+ ( get-bird-y bird) bird-height  ) )
                                              (make-posn (mypipe-x (car pipes))  0)
                                               ( make-posn  (+ ( mypipe-x (car pipes)) pipe-width  ) ( mypipe-gap-y (car pipes) )   ))
                                               
                  ( check-collision-rectangles (make-posn ( mybird-x bird ) ( get-bird-y bird )  )
                                               (make-posn  (+ ( mybird-x bird ) bird-width ) (+ ( get-bird-y bird) bird-height  ) )
                                               ( make-posn ( mypipe-x (car pipes) ) (+ ( mypipe-gap-y (car pipes) ) pipe-self-gap )  )
                                                ( make-posn  (+ ( mypipe-x (first pipes)) pipe-width  ) ground-y  ))
...
(define (next-state state)
  (  mystate (  next-state-bird (get-bird state) initial-gravity )
...
(define (draw-frame state)
  ( place-image bird-image ( + (mybird-x (get-bird state)) (quotient bird-width 2)) ( + (mybird-y (get-bird state)) (quotient bird-height 2))
...
(define (place-pipes pipes scene)
  (if (null? pipes) scene
      ( place-pipes (cdr pipes) (place-image pipe-image (+ (get-pipe-x (car pipes)) (quotient pipe-width 2)) (- (mypipe-gap-y (car pipes)) (quotient pipe-height 2))
                                 (place-image pipe-image (+ (get-pipe-x (car pipes)) (quotient pipe-width 2)) (+ (mypipe-gap-y (car pipes)) (+ pipe-self-gap (quotient pipe-height 2))) scene
  )
             )
                                             )))
